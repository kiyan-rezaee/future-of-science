...
1. Collecting the Publication Corpus
	...
	- By default the script collects the full historical span (1827-2025) which is time- and resource-intensive. You can [edit `config.yaml` and set your desired historical span](https://github.com/kiyan-rezaee/future-of-science/blob/main/config.yaml#L9).
...
3. Building the Per-Year Publication Corpus
	- You should run the `publication_corpus_creation.py` script which builds a database table for each year in your desired historical span in parallel. You can set the number of workers in [`config.yaml`](https://github.com/kiyan-rezaee/future-of-science/blob/main/config.yaml#L35).
